...
async def tambah(data: DataModel, request: Request):
    perintah.execute("INSERT INTO siswa (nama, hobi) VALUES (%s, %s)",
    (data.nama, data.hobi))
    atas = headers(request)
    db.commit()
    db.close()
...
async def hapus(id: int, request: Request):
    perintah.execute("DELETE FROM siswa WHERE id = %s", (id,))
    atas = headers(request)
    db.commit()
    db.close()
...
async def update(id: int, data: DataModel, request: Request):
    ...
    db.commit()
    db.close()
    return JSONResponse(content={"massage": "data berhasil di perbarui"}, headers=atas)
...
async def update_all(id: int, data: DataModel, request: Request):
    perintah.execute("UPDATE siswa SET nama = %s, hobi = %s WHERE id = %s",
